...
把散落在文件夹、手机和聊天记录里的照片，整理成一本有顺序、有呼吸、能反复翻看的离线画册。它不是把照片做成漂亮模板，而是帮你把一组照片重新放回当时的时间、关系和心情里。

每张照片都可以问自己一个问题：

> 当时为什么按下快门？

有时答案是一个人回头的样子，有时是一顿饭、一段路、一片云，也可能只是某个普通下午忽然值得被留下。整理照片不是收尾工作，它本身就是回忆的一部分。时不时翻出来看一下，照片就不会只躺在硬盘里慢慢发霉。

`photo-album-skill` 会把照片和文字做成自然、好翻的单文件 HTML 画册，也能逐页导出成小红书或朋友圈图片组。

默认体验很简单：给一个目录，先出一版；标题、主题和叙事之后再改。

## 适合用来做什么

- 给旅行、聚会、家庭日常、宠物、街拍或某个普通周末做一本可翻阅的照片书。
- 把散落照片按时间线、地点、人物或情绪重新整理成体系。
- 为小红书、朋友圈、公众号配图导出一组顺序清楚的图片。
- 压缩别的软件生成的超大 standalone HTML，让相册适合离线阅读。
- 给未来的自己留一份“当时为什么拍下这些”的回看入口。
...
不以技术好坏为标准，以"意义"和"人情味"为核心。所有照片先问一句：**当时为什么按下快门？**
