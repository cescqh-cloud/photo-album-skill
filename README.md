...
- **安全裁切优先**：默认保留照片原构图，路牌、合照、竖图、食物、航拍和云海不会为了填满卡片被硬裁。
...
- **连续电影感动效**：页面以前景、当前景和后景连续交接，静止时保留克制的呼吸运动；手机端自动减弱，系统开启“减少动态效果”时退化为短淡入。
...
- 根据横竖比例、文字长度和前后照片自动组合页面，宽幅风景优先使用 `panorama`。
- 为图片写入 `aspect-*` 与 `fit-*` 类；默认完整展示，只有封面和明确出血图才裁切铺满。
- 将图片内嵌为 data URI，输出单个离线 HTML。
- 加入统一的按钮、键盘和触控导航，以及进度、主题切换、图片放大和打印样式。
- 默认使用原生 CSS 与 JavaScript 的分层空间动效，不需要联网或额外动画库；快速连续翻页会合并请求，避免动画互相打架。

默认压缩档位是 `balanced`：最长边 1600px、质量 80，兼顾离线阅读速度和清晰度。常用档位：

```powershell
# 文件更小，适合微信转发或快速预览
python scripts/build_album.py "D:\Photos\周末散步" --profile fast

# 更清晰，适合大屏或 PDF 前置版本
python scripts/build_album.py "D:\Photos\周末散步" --profile quality

# 也可以手动覆盖
python scripts/build_album.py "D:\Photos\周末散步" --max-edge 2000 --quality 84
```
